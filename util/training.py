...
def one_run(train_dict,freeze,lr_list,lr_array):
    ...
    while not done:
        ...
        print(f'train mse_loss: {np.array(epoch_train_mse_loss).mean()}   train point_loss: {np.array(epoch_train_point_loss).mean()}')
        print(f'test mse_loss: {np.array(epoch_test_mse_loss).mean()}   test point_loss: {np.array(epoch_test_point_loss).mean()}')
